...
if(!match){(controller()).view(view_error);console.log('NOTE','routes['+path+'] not found');return;}
...
method='index';if(!$GLOBALS[object]){console.log('NOTE',object+'->'+method+'('+argument+') not found');(controller()).view(view_error);return;}
if(!$GLOBALS[object][method]){console.log('NOTE',$method+'('+argument+') not found');(controller()).view(view_error);return;}
...
function DispatchEvent(elm,evt){var event;if(typeof(Event)==='function'){event=new Event(evt,{bubbles:true,cancelable:false});}else{event=document.createEvent('Event');event.initEvent(evt,true,false);}
elm.dispatchEvent(event);}
if(ENVIRONMENT!='development'){console.log=function(){};}
window.addEventListener('popstate',function(){DispatchEvent(document,'DOMContentLoaded');});document.addEventListener("DOMContentLoaded",function(){Router(window.location.pathname);var hrefs=document.querySelectorAll("[href]");for(var i=0;i<hrefs.length;i++){hrefs[i].addEventListener("click",function(event){event.preventDefault();history.pushState(null,null,this.href);DispatchEvent(document,'DOMContentLoaded');});}});
