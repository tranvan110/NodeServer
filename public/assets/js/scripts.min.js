...
function toggle(elem) {
	...
	else if (element.className.indexOf("hide") >= 0)
...
function SelectsInit() {
	...
	for (var i = 0; i < selects.length; i++) {
		selects[i].addEventListener("click", function () {
			options = this.querySelectorAll(".options span");
			OptionsInit(options);
...
var slideIndex, dots = [], slides = [], thumbs = [];

//////////////////////////////////////////////////////////////////////////////////
function ZoomInit() {
	var zoom = document.querySelector(".zoom");
	var lens = document.querySelector(".zoom-lens");
	var result = document.querySelector(".zoom-res");
	...
	function moveLens(event) {
		var img = document.querySelector(".zoom .active");
		var offset = getOffset(img, img.parentElement);
		...
		lens.style.left = offset.x + x + "px";
		lens.style.top = offset.y + y + "px";
		result.style.backgroundImage = "url('" + img.src + "')";
		result.style.backgroundSize = (img.width * cx) + "px " + (img.height * cy) + "px";
		result.style.backgroundPosition = "-" + (x * cx) + "px -" + (y * cy) + "px";
	}

	function getCursorPos(event, offset) {
		event = event || window.event;
		var offsetPos = offset.getBoundingClientRect();
		var x = event.pageX - offsetPos.left;
		var y = event.pageY - offsetPos.top;
	...
	function getOffset(element, offset) {
		...
		var offsetPos = offset.getBoundingClientRect();
		var x = elementPos.left - offsetPos.left;
		var y = elementPos.top - offsetPos.top;
		return { x: x, y: y };
	}
}
...
function SlidesInit() {
	...
	if (document.querySelector(".zoom"))
		ZoomInit();
	if (document.querySelector(".slides .auto"))
		intervals.push(setInterval(SlideAuto, 2000));
...
//////////////////////////////////////////////////////////////////////////////////
function ThumbsInit() {
	var next = document.querySelector(".thumbs .next");
	var prev = document.querySelector(".thumbs .prev");
...
function ThumbShow(index) {
	for (var i = 0; i < thumbs.length; i++) {
		thumbs[i].parentElement.classList.remove("active");
	};
	if (thumbs[index].parentElement)
